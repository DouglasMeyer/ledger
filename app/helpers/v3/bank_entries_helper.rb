module V3
  module BankEntriesHelper
    ...
    def account_entry_fields(f, account)
      ...
      account_entry = f.object.account_entries.build(
        account_id: account.id,
        amount_cents: 0
      )
      account_entry.amount_cents = account_entries.sum(:amount_cents)
      account_entry.strategy = account.strategy
      if account_entries.first.try(:strategy)
        account_entry.amount = account_entries.first.strategy.value(f.object)
      end

      f.fields_for(:account_entries, account_entries) do |ae|
